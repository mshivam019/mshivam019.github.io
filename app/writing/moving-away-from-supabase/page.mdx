...
And then there's the recent Vercel security breach. April 2026. A compromised AI tool let attackers pivot through OAuth into Vercel's internal systems, exposing customer environment variables. An AI tool was the way in, at a platform that hosts AI products for a living.
...
This is also why I wouldn't swap Supabase Auth for Clerk or Auth0. That just moves the lock-in into someone else's database. Better Auth avoids that. It's a library, not a service, so your user table stays yours.
...
Managed platforms optimize for the 0→1 journey. They make starting easy and growing expensive. The pricing isn't accidental. It's designed so that by the time you hit the limits, you're too deep to leave easily.
